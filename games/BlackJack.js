...
constructor BlackJack(player1, player2){
  super("BlackJack", player1, null);
  ...
  this.playerCardNum = -1;
  //the amount of cards the dealer has -1
  this.dealerCardNum = -1;
  //the money the player has
  //not used for now
  this.money = player1.score;
...
//function to deal out the intial cards
function Deal(){
  var x = GetCardValue();
  var y = GetCardSuit();
  if(x==1){
  ...
  ShowCard(x, y, "player");
  x = GetCardValue();
o/ estingmount of money you would like to bet\nThe default bet is 10.", 10); javascript codes
  y = GetCardSuit();
  if(x>10){
    dealerValue += 10;
  }
...
//function to show the card onto the board
function ShowCard(x, y, name){
  if(name == "dealer"){
